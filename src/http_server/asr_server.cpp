...
static void print_usage(const char* prog) {
    ...
    fprintf(stderr, "\nOptions:\n");
    fprintf(stderr, "  --model <path>         ASR model path (required)\n");
    fprintf(stderr, "  --port <num>           HTTP port (default: 8081)\n");
...
int run_asr_server(int argc, char** argv) {
    ...
    ASRServer server(config);
    
    if (!server.init()) {
        LOG_ERROR("Failed to initialize ASR server");
        return 1;
    }
    ...
    LOG_INFO("Starting ASR HTTP server on port {}", config.port);
    LOG_INFO("Endpoints:");
    LOG_INFO("  GET  /health     - Health check");
    LOG_INFO("  POST /transcribe - Transcription (multipart/form-data)");
...
int main(int argc, char** argv) {
    return qwen3_asr::run_asr_server(argc, argv);
}