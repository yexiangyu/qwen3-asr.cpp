...
static ggml_cgraph* build_graph_conv(EncoderState* state, int n_frames) {
    ...
    ggml_tensor* mel = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_frames, n_mel);
    ggml_set_name(mel, "mel"); ggml_set_input(mel);
    
    ggml_tensor* cur = ggml_conv_2d(ctx, m.conv2d1_w, ggml_reshape_4d(ctx, mel, n_frames, n_mel, 1, 1), 2, 2, 1, 1, 1, 1);
    ...
    int64_t seq_len = cur->ne[0], feat_dim = cur->ne[2] * cur->ne[1];
    cur = ggml_reshape_2d(ctx, ggml_cont(ctx, ggml_transpose(ctx, ggml_reshape_3d(ctx, cur, cur->ne[0], cur->ne[1] * cur->ne[2], 1))), feat_dim, seq_len);
    
    if (m.conv_out_w) cur = ggml_mul_mat(ctx, m.conv_out_w, cur);
    
    ggml_set_name(cur, "embd_conv"); ggml_set_output(cur);
    state->embd_conv = cur;
...
static ggml_cgraph* build_graph_encoder(EncoderState* state, int n_ctx) {
    EncoderModel& m = *state->model;
    ...
    ggml_set_name(inpL, "enc_input"); ggml_set_input(inpL);
    ...
    for (int L = 0; L < n_layer; ++L) {
        ...
        ggml_tensor* attn = ggml_mul_mat(ctx, V, ggml_soft_max_ext(ctx, ggml_mul_mat(ctx, K, Q), nullptr, scale, 0.0f));
    ...
    ggml_set_name(cur, "embd_enc"); ggml_set_output(cur);
    state->embd_enc = cur;
    ggml_build_forward_expand(gf, cur);
    ggml_free(ctx);
    return gf;
...
bool encode(EncoderState* state, const Input& input, AudioFeatures& output, ErrorInfo* error) {
    if (!state || !state->model) { if (error) error->message = "State not initialized"; return false; }
    
    EncoderModel& m = *state->model;
    int n_state = m.hparams.d_model;
    int chunk_sz = 100;
    int n_chunks = (input.n_frames + chunk_sz - 1) / chunk_sz;
    
    std::vector<float> all_conv;
    std::vector<int> chunk_out_lens;
    int total_frames = 0;
    
    for (int c = 0; c < n_chunks; ++c) {
        int start = c * chunk_sz, len = std::min(chunk_sz, input.n_frames - start);
        int out_len = compute_conv_output_len(len);
        chunk_out_lens.push_back(out_len);
        total_frames += out_len;
        
        ggml_cgraph* gf_conv = build_graph_conv(state, len);
        if (!ggml_backend_sched_alloc_graph(state->sched, gf_conv)) { if (error) error->message = "alloc conv failed"; return false; }
        
        ggml_tensor* mel_t = ggml_graph_get_tensor(gf_conv, "mel");
        std::vector<float> chunk_mel(len * input.n_mels);
        for (int i = 0; i < input.n_mels; ++i) for (int j = 0; j < len; ++j) chunk_mel[j + i * len] = input.mel_data[i * input.n_frames + start + j];
        ggml_backend_tensor_set(mel_t, chunk_mel.data(), 0, len * input.n_mels * sizeof(float));
        
        if (ggml_backend_sched_graph_compute(state->sched, gf_conv) != GGML_STATUS_SUCCESS) { if (error) error->message = "compute conv failed"; ggml_backend_sched_reset(state->sched); return false; }
        
        ggml_tensor* embd = ggml_graph_get_tensor(gf_conv, "embd_conv");
        int64_t out_ctx = embd->ne[1], out_state = embd->ne[0];
        std::vector<float> conv_out(out_ctx * out_state);
        ggml_backend_tensor_get(embd, conv_out.data(), 0, out_ctx * out_state * sizeof(float));
        
        std::vector<float> pe(out_ctx * out_state);
        compute_sinusoidal_pe(pe.data(), out_ctx, out_state);
        for (size_t i = 0; i < conv_out.size(); ++i) conv_out[i] += pe[i];
        
        all_conv.insert(all_conv.end(), conv_out.begin(), conv_out.end());
        ggml_backend_sched_reset(state->sched);
    }
    
    ggml_cgraph* gf_enc = build_graph_encoder(state, total_frames);
    if (!ggml_backend_sched_alloc_graph(state->sched, gf_enc)) { if (error) error->message = "alloc enc failed"; return false; }
    
    ggml_tensor* enc_in = ggml_graph_get_tensor(gf_enc, "enc_input");
    ggml_backend_tensor_set(enc_in, all_conv.data(), 0, total_frames * n_state * sizeof(float));
    
    if (ggml_backend_sched_graph_compute(state->sched, gf_enc) != GGML_STATUS_SUCCESS) { if (error) error->message = "compute enc failed"; ggml_backend_sched_reset(state->sched); return false; }
    
    ggml_tensor* embd_enc = ggml_graph_get_tensor(gf_enc, "embd_enc");
    output.n_frames = embd_enc->ne[1];
    output.hidden_size = embd_enc->ne[0];
    output.data.resize(output.n_frames * output.hidden_size);
    ggml_backend_tensor_get(embd_enc, output.data.data(), 0, output.data.size() * sizeof(float));
